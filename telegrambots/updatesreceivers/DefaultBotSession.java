...
import james_gosling.projects.balebotmg.telegrambots.meta.api.methods.updates.GetUpdates;
...
import james_gosling.projects.balebotmg.telegrambots.meta.generics.UpdatesHandler;
import james_gosling.projects.balebotmg.telegrambots.meta.generics.UpdatesReader;
...
public class DefaultBotSession implements BotSession {
    ...
    private class ReaderThread extends Thread implements UpdatesReader {
        ...
        public void interrupt() {
            if (httpclient != null) {
                try {
                    ...
                    Log.w("ME" ,e.getLocalizedMessage());
                }
            }
            super.interrupt();
        }
        ...
        public void run() {
                ...
                synchronized (lock) {
                    if (running.get()) {
                        try {
                            ...
                            Log.d("ME", Objects.requireNonNull("ME" ,e.getLocalizedMessage()));
                            interrupt();
                        } catch (Exception global) {
                            Log.e("ME", Objects.requireNonNull(global.getLocalizedMessage()));
                            try {
                                synchronized (lock) {
            ...
            Log.d("ME" ,"Reader thread has being closed");
        }
        ...
        private List<Update> getUpdatesFromServer() throws IOException {
            ...
            try (CloseableHttpResponse response = httpclient.execute(httpPost, options.getHttpContext())) {
                ...
                Log.e("ME" , Objects.requireNonNull(e.getLocalizedMessage()));
            } catch (SocketTimeoutException e) {
                Log.i("ME" , Objects.requireNonNull(e.getLocalizedMessage()));
            } catch (InterruptedException e) {
                Log.i("ME" , Objects.requireNonNull(e.getLocalizedMessage()));
                interrupt();
        ...
        public void run() {
            ...
            while (running.get()) {
                try {
                    ...
                    Log.d("ME" , Objects.requireNonNull(e.getLocalizedMessage()));
                    interrupt();
                } catch (Exception e) {
                    Log.d("ME" , Objects.requireNonNull(e.getLocalizedMessage()));
                }
            }
            Log.d("ME" ,"Handler thread has being closed");
        }
    }
}
